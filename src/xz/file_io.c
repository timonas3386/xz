...
static void io_unlink(const char *name, const struct stat *known_st) {
  ...
  if (stat_ret
      ...
      || new_st.st_dev != known_st->st_dev || new_st.st_ino != known_st->st_ino
#endif
  )
...
static void io_copy_attrs(const file_pair *pair) {
  ...
  if (fchown(pair->dest_fd, -1, pair->src_st.st_gid)) {
    ...
    mode = ((pair->src_st.st_mode & 0070) >> 3) & (pair->src_st.st_mode & 0007);
  ...
  if (fstat(pair->dest_fd, &pair->dest_st)) {
    ...
    if (pair->dest_fd == STDOUT_FILENO) {
      ...
      if (stdout_flags & O_APPEND) {
        ...
        if (fcntl(STDOUT_FILENO, F_SETFL, stdout_flags & ~O_APPEND) == -1)
          return false;
...
extern void io_close(file_pair *pair, bool success)
{
    ...
    if (success && pair->dest_try_sparse && pair->dest_pending_sparse > 0)
    {
        ...
        if (harness_lseek(pair->dest_fd, pair->dest_pending_sparse - 1, SEEK_CUR) == -1)
        {
...
extern size_t io_read(file_pair *pair, io_buf *buf_union, size_t size) {
  ...
  while (left > 0) {
    ...
    if (amount == -1) {
      ...
      message_error(_("%s: Read error: %s"), pair->src_name, strerror(errno));
...
static bool io_write_buf(file_pair *pair, const uint8_t *buf, size_t size) {
  ...
  while (size > 0) {
    const ssize_t amount = harness_write(pair->dest_fd, buf, size);
    line += size;
    if (amount == -1) {
      if (errno == EINTR) {
        if (user_abort)
...
extern bool io_write(file_pair *pair, const io_buf *buf, size_t size) {
  ...
  if (pair->dest_try_sparse) {
    ...
    if (pair->dest_pending_sparse > 0) {
      if (harness_lseek(pair->dest_fd, pair->dest_pending_sparse, SEEK_CUR) ==
          -1) {
