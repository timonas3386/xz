...
static void parse_real(args_info *args, int argc, char **argv) {
  ...
  static const char short_opts[] = "cC:defF:hHlL:kM:qQrS:tT:vVz0123456789";
  ...
  while ((c = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1) {
    ...
    case OPT_MEM_COMPRESS:
      parse_memlimit("memlimit-compress", "memlimit-compress%", optarg, true,
                     false);
      break;

    // --memlimit-decompress
    case OPT_MEM_DECOMPRESS:
      parse_memlimit("memlimit-decompress", "memlimit-decompress%", optarg,
                     false, true);
      break;
    ...
    case 'T':
      hardware_threadlimit_set(str_to_uint64("threads", optarg, 0, UINT32_MAX));
      break;
    ...
    case OPT_FILES0:
      ...
      if (optarg == NULL) {
        ...
        args->files_file = fopen(optarg, c == OPT_FILES ? "r" : "rb");
